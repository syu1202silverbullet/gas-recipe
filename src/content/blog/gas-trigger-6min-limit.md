...
こんにちは、看護師として働きながらGASで副業をしている凛です。データが増えてきた頃から、自動化したはずのスクリプトが「Exceeded maximum execution time」と言って途中で力尽きる——この6分制限の壁は、GASを使い込むと誰もが一度はぶつかる関門だと思います。私も夜間バッチが毎回こけて、夜勤明けにログを開いては頭を抱えていた時期がありました。

この記事では、当時の私が誰かに聞きたかったことを、そのままQ&A形式でまとめました。

# GAS6分制限を回避する3パターン完全解説

## Q. そもそも、なぜ6分で止まるの？延長はできない？
...
残念ながら「制限そのものを伸ばす」裏技は存在しません。発想を切り替えて、**6分以内で区切って何回かに分けて走らせる**のが正攻法です。最初にこの割り切りができると、あとの実装はすんなり進みます。

## Q. 「区切って回す」って、具体的にはどうやるの？
...
3パターンすべての実装をまとめたのが次のコードです。構文チェックのうえで掲載していますが、「実際の処理をここに書く」の部分はご自身の用途に置き換えてください。
...
設定後、`processInChunks()` を手動実行して動作確認してから本番稼働させましょう。いきなりトリガー任せにしないのがコツです。

## Q. 実運用で気をつけることは？

私が実際に運用しながら学んだポイントを3つ挙げます。
...
迷ったら、実装難易度の低い「分割実行＋LockService」の組み合わせから始めれば十分です。6分制限は「制限を伸ばす」のではなく「区切って回す」——この発想の転換さえできれば、あとはコードを貼って調整するだけ。「自動化したはずなのにまた止まってる」を防ぐ仕組みまで作って、はじめて夜勤明けでも安心して任せられるGASになります。私の夜間バッチが静かに完走するようになったのも、この形に変えてからでした。
...
---
*コードは構文レベルで確認のうえ掲載しています。実行時間はデータ量によって変わるので、まずは少量のデータでお試しください。*
