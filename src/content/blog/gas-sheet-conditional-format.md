...
「条件付き書式って、GASからまとめて流し込めないの？」——シートが増えるたびに同じルールを手でポチポチ設定しながら、そう思ったことはありませんか。

答えから言うと、できます。それも10個のルールを、1回の実行で丸ごと。

こんにちは、看護師をしながらコードを書いている凛です。スプレッドシートに色を手で付けていく作業は、患者さんのバイタル表を蛍光ペンで塗り分けていた新人時代を思い出します。私も副業の売上ダッシュボードで複数シートに同じ条件付き書式を入れたくて、手動だと地味に1時間かかっていました。GASで一括設定にしたら、シート追加時もスクリプト1回で全部適用。ダッシュボード運用が劇的に楽になりました。

## 答え：ルールを「組み立てて、流し込む」だけ

GASで条件付き書式を設定する基本の流れを先に押さえておきましょう。
...
流れはこれだけ。ここからは、実務でそのまま使える10個のルールを順に見ていきます。
...
新しく月次シートを作成したとき、`applyAllRules('2026年6月')`を1回実行するだけで同じ書式が整います。この瞬間がいちばん気持ちいいんですよね。
...
この10ルールを組み合わせれば、スプレッドシートのダッシュボードが一気に見やすくなります。新しいシートに追加するたびに関数を1回実行するだけなので、書式のメンテナンスにかける時間もほぼゼロになります。

## おわりに

「目で見て分かるシート」は、疲れている日のミスを減らしてくれます。数字を読む前に、色が先に教えてくれる——これは紙のバイタル表でもスプレッドシートでも同じでした。夜勤明けの自分にやさしい書式を、元気なうちにGASで仕込んでおきましょう。
...
2児2児のママで現役ナース。夜勤明けの細切れ時間を副業GASに投じ、月5〜8万円の副収入を継続中。「看護師でもコードは書ける」を合言葉に、家事育児とプログラミングを両立する等身大の情報を発信しています。掲載コードは構文・API仕様・ロジックを一つずつ確認したうえで載せています。
