...
こんにちは、夜勤明けのコーヒー片手にコードを書くのが好きな看護師の凛です。副業を始めてから、毎月スプレッドシートを開いて「ファイル→ダウンロード→CSV→会計ソフトにアップロード」と繰り返す作業が、地味に憂うつでした。ちょっとした手間でも積み重なると後回しにしてしまうもの。そこでGASで自動化して、この月イチの面倒ごとを丸ごと手放すことにしました。
