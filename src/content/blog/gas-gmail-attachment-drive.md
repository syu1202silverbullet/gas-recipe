...
こんにちは、凛です。2児のママで現役ナースをしながら、GASで副業をしています。私は副業でネットショップ運営もしているので、毎月十数件の請求書・納品書・明細PDFがGmailに届きます。以前は週末にまとめて手動でDriveに振り分けていたのですが、これが地味に1時間以上かかる苦行でした。
...
ラベル管理を忘れると、トリガーで繰り返し実行するたびに同じファイルが保存されます。必ず処理済みのスレッドに「Drive保存済み」ラベルをつけて、次回以降はスキップする設計にしましょう。

### つまずき2：ファイル名が重複してしまう

同じ名前の添付ファイルが複数届いた場合、Driveに同名ファイルが複数できてしまいます。ファイル名に日付や送信者名を付け加える工夫（ポイント2）を入れれば解決できます。

### つまずき3：不要な画像ファイルまで保存される

メールの署名に含まれる画像、Gmailの内部ヘッダー画像なども「添付ファイル」として検出されることがあります。保存したい拡張子だけに絞るフィルタ（ポイント3）を必ず入れてください。
...
| ポイント | 実装内容 | 効果 |
|---|---|---|
| 基本コード | 10行でGmail→Drive保存 | 手動保存ゼロ |
| 年月フォルダ | 月別サブフォルダを自動作成 | 確定申告がスムーズ |
| ファイル名加工 | 日付+送信者名+元ファイル名 | あとから探しやすい |
| 拡張子フィルタ | PDFのみなど指定 | 不要な画像が混入しない |
| ログ記録 | スプシにファイルURLを保存 | ワンクリックでファイルに移動 |
