...
こんにちは、本業は看護師、副業でGASを書いている凛です。

以前の私の職場では、物品在庫の共有がこんな流れでした。在庫担当の看護師がスプレッドシートを更新して、変更があったことをメールで周知。受け取った側はわざわざスプシを開いて確認する。たったこれだけのことに、双方の手間と「確認したかどうか分からない」モヤモヤが付きまとっていました。

それが今は、スタッフがスマホでURLをひとつ開くだけ。スプレッドシートの中身がそのままWebページとして表示されるので、メールでの周知そのものが不要になりました。

このBeforeからAfterへの変化を支えているのが、GASの`HtmlService`です。GASで作ったWebアプリに動的なデータを表示したい、でもHTMLとGASの値の橋渡しがよく分からない——そこで止まってしまう方、多いんじゃないでしょうか。正直に白状すると、私もWebアプリ初挑戦のとき、`<?= variable ?>` という見慣れない記法を見て怯みました。でも慣れてしまえば、だいたい10個のパターンを使い回すだけでほぼ全部カバーできます。

この記事では、その10パターンを実装コード付きで順番に見ていきます。「これ知ってたら最初からもっと楽だった」という実務目線でまとめました。

## そもそもGASがHTMLを返せる、という話
...
この流れさえ押さえておけば、あとは10個のパターンを組み合わせるだけです。
...
一番の働き者は3番目の`createTemplateFromFile`。スプレッドシートのデータをHTMLに渡して動的なページを作るとき、これが必須になります。
...
HTMLファイルの作り方は簡単で、GASエディタを開いて、左上の「+」→「HTML」をクリック。ファイル名を`index`にして作成します。
...
GASの変数をHTMLに埋め込む方法です。これを覚えた瞬間、作れるものが一気に増えます。
...
ここからが本番です。冒頭でお話しした在庫確認ツールも、この章のパターンの組み合わせでできています。

### 例5：スプレッドシートのデータをHTML表示

実務で一番よく使うパターン。スプレッドシートのデータを取得して、テーブルとして表示します。
...
1行目をヘッダー（`<th>`）、それ以降をデータ行（`<td>`）として表示する工夫が入っています。スプレッドシートをそのままWebページに公開できるわけです。在庫リストのWebアプリ化も、ベースはこのコードでした。スプレッドシートを更新すれば自動でWebに反映される。それだけのことなのに、現場の「確認の手間」が目に見えて減りました。
...
CDNを使えば余計なファイルを作らずにデザインが整います。凝ったデザインにする時間がない日ほど、この手抜き（といって悪ければ効率化）が効いてきます。
...
ページが開いたとき、`window.onload` が発火して`getLatestData`を呼び出し、返ってきたデータをHTMLに変換して表示します。「URLを開けば常に最新の状態が見える」ツールは、このパターンで作ります。

## アプリを育てるための2パターン（例9〜例10）

### 例9：別ファイルにJS/CSSを分割

HTMLが大きくなると、正直かなり見づらくなります。JS・CSSを別ファイルに分割して管理しやすくする方法です。
...
`include`関数でHTMLファイルの中身を文字列として取得して、`<?!= ?>`で埋め込んでいます。これでindex.html・style.html・script.htmlの3ファイル構成で管理できます。あとから直すときの気楽さが全然違いますよ。
...
看護師がスマホで記録を入力するWebアプリを作った経験から言うと、モバイル対応の優先度はかなり高いです。病棟でPCに向かえる時間は限られているので、スマホで入力できる設計にしておくと実際に使ってもらえます。逆に言えば、PCでしか使えないツールは、どれだけ便利でも現場では開いてもらえません。
...
`<?!= ?>`（エスケープなし出力）にユーザーが入力した値を渡すと、HTMLを埋め込まれて悪意のあるコードが実行されるリスクがあります。ユーザー入力は必ず `<?= ?>` で出力（自動エスケープ）、GASコード内で安全に生成したHTMLだけを `<?!= ?>` で出力する。この使い分けを守れば安全です。
...
参考までに、私が職場で回してきたWebアプリを、どの例の組み合わせで作ったかという視点で紹介します。

**物品在庫確認ツール**は、冒頭のBefore/Afterの主役です。スプレッドシートで在庫管理し、GAS WebアプリでURL化して、スタッフのスマホのホーム画面に追加してもらいました。中身は例5（スプシデータをHTML表示）＋例6（CSS適用）＋例10（モバイル対応）の組み合わせ。在庫担当者がスプシを更新すれば、他のスタッフはURLを開くだけで最新状態を確認できます。

もうひとつは**申し送りメモ確認ツール**。前の番から引き継いだメモをシフト交代時にスマホで確認するためのもので、例3（テンプレート変数差し込み）＋例8（ページ読込時にデータ取得）で組みました。

どちらも「スプレッドシートに入力 → URLで確認」というだけのシンプルな設計です。それでも現場では相当な時短になりました。凝ったことをしなくても、Before/Afterの差はちゃんと出ます。

## ここまで書いて思うこと

10例を一気に並べましたが、全部を最初から覚える必要はまったくありません。例1で「動いた！」を体験して、例3のテンプレート式と例5のスプシ表示を押さえれば、実務ツールの土台はもうできています。フォーム入力が必要になったら例7、スマホで使ってもらうなら例10、と必要になった順に引き出しを開けていけば十分です。

私自身、最初の在庫ツールを作ったときに使ったのは10例のうち3つだけでした。それでも「メールで周知」という長年の習慣がひとつ消えたのだから、費用対効果は抜群だったと思います。サーバー不要・完全無料で、5分でURLを発行できるのがGAS Webアプリの強み。あなたの職場の「毎回メールで確認しているアレ」も、URLひとつに置き換えられるかもしれません。
...
2児2児のママで現役ナース。夜勤明けの細切れ時間を副業GASに投じ、月5〜8万円の副収入を継続中。「看護師でもコードは書ける」を合言葉に、家事育児とプログラミングを両立する等身大の情報を発信しています。掲載コードは構文・API仕様・ロジックを机上で確認したうえで載せていますが、お使いの環境での動作までは保証できないので、必ずテスト実行してからお使いください。
