...
こんにちは、凛です。副業でWebエンジニアをしていると、毎月クライアントへの領収書発行が地味に面倒ですよね。「Wordのテンプレを開いて、日付を変えて、名前を変えて、PDF出力して、メールに添付して…」この作業、全部GASに任せられます。

今日は**スプレッドシートのテンプレートに入力してボタンを押すだけで、領収書PDFを自動採番・Drive保存・Gmail送信まで完結するGAS**を、動作検証済みの完全版コードで解説します。

## こんな悩みありませんか？

- 毎月同じ宛名の領収書を手作業で作り直している
- 領収書の通し番号を自分で管理していて、たまにミスる
- 発行したPDFがあちこちに散らばって、確定申告時に探し回る
- クライアントへの送り忘れが月に1回くらい発生している

私も副業初年度は完全手作業でした。「どうせ月数枚だから」と思っていたのに、クライアントが増えると一気に面倒になります。GASで仕組みにしてからは、発行作業が**1件30秒**で終わっています。
...
`flush()` を1行挟むだけで確実に解消されます。**地味に忘れがちな落とし穴**です。

### 採番はスクリプトプロパティで管理

「今日何枚目か」を `PropertiesService.getScriptProperties()` で保存しています。
スプレッドシートに管理列を作る必要がなく、シートのデザインを汚しません。
...
これで「どのクライアントにいつ何円の領収書を出したか」が自動で記録されます。確定申告にそのまま使えます。
...
- スプシに宛名・金額を入力 → メニューから実行 → PDF保存・送信まで全自動
- 領収書番号はスクリプトプロパティで日付連番管理（シートを汚さない）
- `SpreadsheetApp.flush()` を PDF 出力前に挟むのが最重要ポイント
- 月別フォルダ自動作成・発行履歴の自動記録まで拡張可能
- テンプレートをコピーして初回承認するだけで誰でもすぐ使える

副業のちょっとした事務作業こそ、GASで仕組みにする価値が高い領域です。一度動かしてしまえば、毎月「あの作業やらなきゃ」というストレスが消えます。
