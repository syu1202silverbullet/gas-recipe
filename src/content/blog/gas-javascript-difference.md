...
こんにちは、看護師をしながら副業案件でGASを書いている凛です。ふだん副業案件でGASをメインに書いていて、最初にJavaScriptと比べて「**この違い、知っておきたかった…**」と思った点がいくつかあります。今日は**GASとJavaScriptの違い7つ**を、初心者向けに図解ベースでまとめます。
...
GASプロジェクトを開いたとき、V8ランタイムになっているかを確認しましょう。古いプロジェクトはRhinoランタイムのままになっていることがあります。「プロジェクトの設定」の「Chrome V8ランタイムを有効にする」をオンにしてください。V8にするとconst/let/アロー関数などのモダン構文が使えるようになります。

### コツ2：非同期が不要なことをメリットとして活かす

ブラウザJSに慣れていると「awaitがないと不便」と感じますが、GASでは全て同期で返ってくるので、awaitを書く必要がなく、コードがシンプルになります。asyncを使わずに書けるので、初心者にとってはむしろ学習コストが低いとも言えます。

### コツ3：LocalStorageの代わりにPropertiesServiceを覚える

ブラウザJSではデータの保存にlocalStorageを使いますが、GASでは`PropertiesService`を使います。スクリプトプロパティはAPIキーや設定値の保存に使い、ユーザープロパティは実行ごとの状態保存に使うのが基本パターンです。

## つまずきやすいポイント

### つまずき1：fetchが使えないとわかってうまく代替できない

ブラウザJSの`fetch`は非同期ですが、GASの`UrlFetchApp.fetch`は同期です。Promiseのチェーンを書いている人は、awaitを取り除いて直接返り値を使うだけでほぼ同じコードが動きます。

### つまずき2：タイムゾーンのズレ

`new Date()`をそのまま使うと、GASのサーバーのタイムゾーン（UTC）で動くことがあります。日本時間で処理したい場合は、プロジェクト設定のタイムゾーンを`Asia/Tokyo`にするか、`Utilities.formatDate(date, 'Asia/Tokyo', 'yyyy/MM/dd HH:mm')`のようにタイムゾーンを明示的に指定しましょう。

### つまずき3：6分制限を意識せずに重い処理を書いてしまう

ブラウザJSでは実行時間制限がないため、GASに移行した際に6分制限を忘れがちです。ループで大量のデータを処理するコードを書くときは、最初から「1回の実行で何件まで」を意識した分割設計にしましょう。
