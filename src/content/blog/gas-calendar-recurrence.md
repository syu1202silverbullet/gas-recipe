...
凛です。看護師をしながらコードを書く、ちょっと変わった2児の母です。毎年12月になると「来年の定例予定を全部カレンダーに入れ直す」という地味な作業に追われていました。毎週月曜の打ち合わせ、毎月第3水曜の振り返り——一件ずつ手で登録していくと、それだけで小一時間。この繰り返し入力をGASに任せられないか、と考えたのが今回のテーマの出発点です。

今回のテーマは「GASでGoogleカレンダーに繰り返し予定を自動作成する方法」です。

カレンダー標準の繰り返し機能でも定例予定は作れますが、「祝日は除く」「第3水曜だけ」といった細かい条件になると途端に扱いづらくなります。こうした例外込みのルールをコードで表現できるのがGASの利点で、年初に1回実行するだけで1年分の予定をまとめて用意できます。
