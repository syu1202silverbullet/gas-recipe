...
## 関連記事
- [GASでGoogleフォームの回答をスプシに自動整形・転記する](/blog/gas-form-sheet-auto/)：フォームの回答をスプレッドシートに自動で整形・転記する方法。この記事とセットで使うと回答管理がぐっと楽になります。
- [GASでGoogleドライブのファイルを自動整理する](/blog/gas-drive-auto-organize/)：Driveにたまったファイルを条件ごとに自動でフォルダ分けする基本テクニック。整理の考え方の土台になります。
- [フォーム送信者へ自動返信メールを送るGAS｜申込確認・問い合わせ即レスの定番](/blog/gas-form-auto-reply/)：フォーム送信をきっかけに自動返信メールを送る仕組みの詳しい解説。受付メールをもっと作り込みたい方向けです。
